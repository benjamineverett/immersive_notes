...
def string_combinations(alphabet, n):
    return list(itertools.combinations(alphabet, n))
...
def permutations_in_dict(string, words):
    return list(word for word in (list(''.join(letters) for letters in list(itertools.permutations(string)))) if word in words)
